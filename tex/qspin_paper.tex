...
%
We choose to first focus on the MBL phase. Re-setting the ramp speed \texttt{v} to unity is required for calculating the correct eigensystem of the Hamiltonian at the end of the ramp, since $v$ is a parameter of $H(t;v)$ (see \texttt{line 77}). We want the initial state to be as close as possible to an infinite-temperature state within the given symmetry sector. To this end, we can first calculate the minimum and maximum energy, \texttt{Emin} and \texttt{Emax} of the spectrum of $H_\mathrm{MBL}(t=0)$. This is achieved using the \texttt{hamiltonian} attribute method \texttt{eigsh}, see \texttt{lines 70-71}, which is a wrapper for \href{http://docs.scipy.org/doc/scipy-0.14.0/reference/generated/scipy.sparse.linalg.eigsh.html}{\texttt{scipy.sparse.linalg.eigsh}}, cf.~also~App.~\ref{app:doc}. Then, by taking the `centre-of-mass' we obtain a number, \texttt{E\_inf\_temp}, which is as represents the infinite-temperature energy up to finite-size effects, \texttt{line 72}. The optional argument \texttt{k=2} ensures that only two eigenstates are calculated. To determine which ones, the argument \texttt{which="BE"} specifies  them to be the two states at \texttt{B}oth \texttt{E}nds of the spectrum. Convergence of the underlying diagonalisation algorithm is enforced by explicitly specifying the number of maximal iterations: \texttt{maxiter=1E10}. Since we do not need the eigenstates, we use \texttt{return\_eigenvectors=False}. Last, the \texttt{**eigsh\_args} is a standard Python way of reading off the arguments by name from a dictionary, here \texttt{eigsh\_args}.
\lstinputlisting[firstline=67, lastline=72, firstnumber=67]{example1.py}
%
The initial state \texttt{psi\_0} is then that eigenstate of $H_\mathrm{MBL}(t=0)$, whose energy is closest to \texttt{E\_inf\_temp}. This time, we only need one eigenstate, and hence \texttt{k=1}. To pick up the desired eigenstate, we make use of another optional argument: \texttt{sigma=E\_inf\_temp}.
