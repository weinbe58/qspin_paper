...
\noindent\emph{Code Analysis---} Once again we start out by loading a set of libraries we will need to procede with the calculation of the MBL fermions. 
\lstinputlisting[firstline=2, lastline=7]{\MBLcode}
Some of these libraries and functions we have seen before but in this example we introduce the \texttt{ops\_dict} which defines an operator which can be parameterized by many parameters as opposed to the Hamiltonian which is only parameterized by time. Also, in this script we will be doing many different disordered realisations in one simulations, therefore we will use \texttt{NumPy}'s random number library to produce the random sampling. We load \texttt{uniform} to generate the uniformly distributed random potential as well as \texttt{choice} which we will use to estimate the uncertainties of the disorder averages using a bootstrap re-sampling procedure which we will explain later. In order to time how long each realization takes we will use the \texttt{time} function from python's \texttt{time} library. After importing all the required libraries and function we set up the parameters for the simulation including number of realizations, and physical parameters \texttt{J}, \text{U}, number of up and down fermions etc.
\lstinputlisting[firstline=9, lastline=24]{\MBLcode}
Next we can set up the basis, introducing here the \texttt{tensor\_basis} class. The \texttt{tensor\_basis} class takes $n$ basis objects which it then uses to construct the matrix elements in 
\begin{equation}
\mathcal{H} = \mathcal{H}_1\otimes\mathcal{H}_2\otimes\cdots\otimes\mathcal{H}_n.
\end{equation}
Here we will consider the case where we have two Hilbert spaces, one for fermions with spin up and one for fermions with spin down\footnote{to construct a \texttt{tensor\_basis} object in general: \texttt{t\_basis = tensor\_basis(basis\_1,basis\_2,...,basis\_n)}}. 
\lstinputlisting[firstline=25, lastline=28]{\MBLcode}
Next we can set up the coupling and operator lists
\lstinputlisting[firstline=29, lastline=51]{\MBLcode}
Notice here that the \texttt{"|"} characters is used to separate the operators which belong to the up (left side of tensor product) and down (right side of tensor product) Hilbert spaces in the tensor product. If no string in present the operator is assumed to be the identity. The index list, on the other hand, does not have to have anything separating the two sides of the tensor product it is assumed that the character lines up with the correct site index when the "|" characters are removed. 
...
\lstinputlisting[firstline=52, lastline=56]{\MBLcode}
...
\lstinputlisting[firstline=57, lastline=63]{\MBLcode}
...
\lstinputlisting[firstline=76, lastline=81]{\MBLcode}
...
\lstinputlisting[firstline=82, lastline=85]{\MBLcode}
