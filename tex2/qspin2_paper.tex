...
\begin{itemize}
	...
	\item find a ground state of a Hamiltonian
...
Suppose you have a model, and you are trying to determine its long-time thermalisation behaviour, from which in general you can infer information about integrability. One way to proceed is to conceive a numerical experiment: if we subject the system to periodic driving, this will lift energy conservation. Then, in the non-integrable case, it will absorb energy from the drive until the state reaches infinite-temperature. On the contrary, in the integrable scenario unlimited energy absorption will be inhibited by the existence of the additional conservation laws, and the system is likely to get stuck at some energy density corresponding to a finite temperature. Hence, the long-time dynamics of out-of-equilibrium many-body systems can be used to extract useful information about the complexity of the underlying Hamiltonian.

Below, we take the transverse field Ising model and promote the spin-$1/2$ operators to spin-$1$. There is no known simple mapping to a quadratic Hamiltonian for spin-$1$ and, therefore, the model is likely no longer integrable. Here, we study thermalisation through energy absorption by periodically driving the two spin systems and demonstrate the difference in the long-time heating behaviour between the two. To this end, let us define two operators
\begin{equation}
...
Since $H_{zz}$ is not the only observable, one might worry that some of the information contained in the quantum is lost. Hence, we also look at the entanglement entropy density, which is a property of the quantum state itself:
\begin{equation}
...
\end{equation}
of subsystem A, defined to contain the left half of the chain and $|\mathrm{A}|=L/2$. We denoted the reduced density matrix of subsystem A by $\rho_\mathrm{A}$, and $\mathrm{A^c}$ is the complement of A. To obtain a dimensionless quantity, we normalise the entanglement entropy by the corresponding Page value[CITE].
...
\noindent\emph{Code Analysis---}Now we explain how to carry out the proposed study using QuSpin. We assume the reader has basic familiarity with QuSpin to set up simple time-dependent Hamiltonians. As is customary, we begin by loading the required Python libraries and packages:
\lstinputlisting[firstline=1, lastline=7, firstnumber=1]{\Spincode}
Then, we define the model parameters. Note that we chose all physical couplings to be unity, so the only parameters are the drive frequency and the drive amplitude, which is set equal to the driving frequency [this is chosen arbitrarily]. Also, since the Hilbert space dimensions for the spin-$1/2$ and spin-$1$ systems scale differently with the length of the chain, we shall keep two variables, labelled by \texttt{L\_12} and \texttt{L\_1}, respectively. In general, all variables indexed by \texttt{\_12} and \texttt{\_1} in the code refer to spin-$1/2$ and spin-$1$ quantities, respectively. 
\lstinputlisting[firstline=9, lastline=13, firstnumber=9]{\Spincode}
Let us now define the spin bases. To this end, we make use of the \texttt{spin\_basis\_1d} constructor. Notice the optional argument \texttt{S}, which accepts a string (integer or half-integer) to specify the spin vector size. Requesting symmetry blocks works as usual, by using the corresponding optional arguments.
\lstinputlisting[firstline=15, lastline=20, firstnumber=15]{\Spincode} 
...
\lstinputlisting[firstline=22, lastline=33, firstnumber=22]{\Spincode}
...
\lstinputlisting[firstline=35, lastline=41, firstnumber=35]{\Spincode}
...
\lstinputlisting[firstline=44, lastline=46, firstnumber=44]{\Spincode}
...
\lstinputlisting[firstline=47, lastline=51, firstnumber=47]{\Spincode}
...
\lstinputlisting[firstline=52, lastline=61, firstnumber=52]{\Spincode}
...
\lstinputlisting[firstline=63, lastline=66, firstnumber=63]{\Spincode}
...
\lstinputlisting[firstline=67, lastline=69, firstnumber=67]{\Spincode}
...
\lstinputlisting[firstline=70, lastline=72, firstnumber=70]{\Spincode}
