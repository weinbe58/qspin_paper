...
\lstinputlisting[firstline=1, lastline=4]{example6.py}
First, we define the models parameters.
\lstinputlisting[firstline=5, lastline=8]{example6.py}
...
\lstinputlisting[firstline=9, lastline=10]{example6.py}
...
\lstinputlisting[firstline=11, lastline=11]{example6.py}
...
\lstinputlisting[firstline=12, lastline=14]{example6.py}
...
In order to specify the types of the on-site single-particle operators, we use operator strings. For instance, the transverse field operator $\mathcal{O}=g\sum_{j=0}^{L-1}\sigma^z_j$ becomes \texttt{['x',h\_field]}, while the two-body interaction is \texttt{['zz',J\_zz]}. It is important to notice that the order of the letters in the operator string corresponds to the order the operators are listed in the site-coupling lists. Putting everything into one final list yields:  
\lstinputlisting[firstline=15, lastline=16]{example6.py}
In QuSpin, the user can define both static and dynamic operators. Since this example does not require any time evolution, we postpone the explanation of how to use dynamic lists to Sec.~???, and use an empty list instead.
\lstinputlisting[firstline=17, lastline=17]{example6.py}
...
\lstinputlisting[firstline=18, lastline=19]{example6.py} 
...
\lstinputlisting[firstline=20, lastline=21]{example6.py} 
...
\lstinputlisting[firstline=22, lastline=23]{example6.py}
...
\lstinputlisting[firstline=24, lastline=26]{example6.py}
Let us ficus on the case of periodic boundary conditions \texttt{PBC=1} first. 
\lstinputlisting[firstline=27, lastline=27]{example6.py}
In the fermion model, we have two types of two-body terms: hopping terms $c^\dagger_{i}c_{i+1} - c_{i}c^\dagger_{i+1}$, and pairing terms $c^\dagger_{i}c^\dagger_{i+1} - c_{i}c_{i+1}$. While QuSpin allows any ordering of the operators, for te sake of completeness we set a convention: the site indices grows to the right. Du to the opposite signs in the terms resulting from the fermion statistics, we have to code the site-coupling lists for all four terms separately. This is analogous to the spin-spin interaction above:
\lstinputlisting[firstline=28, lastline=32]{example6.py}
...
\lstinputlisting[firstline=33, lastline=34]{example6.py}
...
\lstinputlisting[firstline=35, lastline=45]{example6.py}
The definition of the basis is the same, except that this time, we need all even particle number sectors:
\lstinputlisting[firstline=46, lastline=47]{example6.py}
...
\lstinputlisting[firstline=48, lastline=50]{example6.py}
Constructing and diagonalising the fermion Hamiltonian is the same as for the spin-$1/2$ system. Note that one can disable the automatic built-in checks for particle conservation \texttt{pcon=False} and all other symmetries \texttt{check\_symm=False} if one wishes to suppress the checks. 
\lstinputlisting[firstline=51, lastline=55]{example6.py}
...
\begin{equation}
	H = -t\sum_{\sigma,i=0}^{L-1} c^\dagger_{\sigma i}c_{\sigma i+1} - c_{\sigma i}c^\dagger_{\sigma i+1} +U\sum_{i=0}^L n_{\uparrow i}n_{\downarrow i} + \sum_{\sigma, i=0}^L V_i n_{\sigma i}\label{eq:FM_ham}
\end{equation}
...
\noindent\emph{Code Analysis---}...
\lstinputlisting[firstline=1, lastline=55]{example9.py}
...
\subsection{Interacting Bosons on Translationally Invariant Ladder}
...
\begin{itemize}
	...
	\item measure entanglement entropy of ladder system 
\end{itemize}
...
If we consider a translational invariant ladder that implies that the Hamiltonian factorizes into different many-body momentum blocks similar to what was discussed in Sec.~\ref{subsec:free_particle} but slightly different as we consider translations of the many-body fock states as opposed to the single particle states[CITE Anders review]. In this section instead of projecting the operators to momentum space as was done in Sec.~\ref{subsec:free_particle}, we will project the wavefunction to the different symmetry sectors and evolve each of the projections separately under the Hamiltonian for that symmetry sector. Then each of the Block wavefunctions are projected back to the local Fock space basis and summed to recover the evolve state which one can then look at local quantities like the density and entanglement.
...
\noindent\emph{Code Analysis---}...
\lstinputlisting[firstline=1, lastline=55]{example8.py}
...
\noindent\emph{Code Analysis---}...
\lstinputlisting[firstline=1, lastline=55]{example7.py}
...
\begin{appendix}
	...
	\lstinputlisting[label=code:ex4,caption=The Spectrum of the Transverse Field Ising Model and the Jordan-Wigner Transformation]{example6.py}
	\newpage
