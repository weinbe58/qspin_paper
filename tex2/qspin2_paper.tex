...
In one dimension, the TFI Hamiltonian can be mapped to spinless $p$-wave superconducting fermions via the Jordan-Wigner (JW) transformation~\cite{sachdev_book,dziarmaga_10}\cyan{one of the first solutions is P.Pfeuty 1979}:
\begin{equation}
...
Some of the additional features we would like to consider in future versions include: a basis class dedicated to single particle lattice physics allowing for more efficient and intuitive way of implementing single particle Hamiltonians, functionality for doing Linblad and other kinds of dynamics with density matrices, finite temperature Lanzcos methods, class for calculating correlation functions, and overall improving the efficient of all of the functionality more generally. 
