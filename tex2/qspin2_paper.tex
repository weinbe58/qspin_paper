...
\cyan{
\begin{itemize}
	\item update example codes to new release
	\item run all tests for code, and all example codes for doc
	\item put all examples in doc, once they are ready
	\item update spinfull fermion doc once ready
	\item update links to doc in appendix	
	\item insert missing MBL references
...
Motivated by recent experiments~\cite{Schreiber15,ovadia_15,bordia_16,smith_16,choi_16,lueschen_17} we explore MBL in the context of fermions using QuSpin. The model we will consider is the Fermi-Hubbard model with quenched random disorder which has the following Hamiltonian:
\begin{equation}
...
The complete code including the lines that produce Fig.~\ref{fig:example9} is available in here.
...
The example scripts for all QuSpin examples can be downloaded under:\\
