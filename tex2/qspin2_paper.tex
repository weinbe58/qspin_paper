...
\begin{itemize}
	\item support for fermion, boson and higher-spin Hamiltonians.
	\item basis constructor classes for user-defined symmetries.
	\item blocks class to automatically handle splitting the evolution over various symmetry sectors of the Hilbert space.
	\item \texttt{quantum\_operator} class to define parameter-dependent Hamiltonians.
	\item user-customisable \texttt{evolve()} routine to handle user-specified linear and non-linear equations of motion.
	\item \texttt{quantum\_LinearOperator} class to efficiently apply an operator to a state without constructing the operator.
\end{itemize}
