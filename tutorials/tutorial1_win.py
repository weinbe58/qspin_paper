...
def realization(vs,H_XXZ,basis,real):
	...
	Emin,Emax=H_MBL.eigsh(time=0.0,k=2,which="BE",maxiter=10000,return_eigenvectors=False)
	E_inf_temp=(Emax+Emin)/2.0
	# calculate nearest eigenstate to energy at infinite temperature
	E,psi_0=H_MBL.eigsh(time=0.0,k=1,sigma=E_inf_temp,maxiter=10000)
	...
	Emin,Emax=H_ETH.eigsh(time=0.0,k=2,which="BE",maxiter=10000,return_eigenvectors=False)
	E_inf_temp=(Emax+Emin)/2.0
	# calculate nearest eigenstate to energy at infinite temperature
	E,psi_0=H_ETH.eigsh(time=0.0,k=1,sigma=E_inf_temp,maxiter=10000)
	psi_0=psi_0.reshape((-1,))
...
#
##### Joblib must be called within this if-statement for windows #####
